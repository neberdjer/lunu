...
use lunu_core::consts::auth::{API_KEY_HEADER, BEARER_PREFIX, SESSION_COOKIE};
...
impl FromRequest for AuthUser {
	...
	fn from_request(req: &HttpRequest, _payload: &mut Payload) -> Self::Future {
		...
		Box::pin(async move {
			...
			Ok(AuthUser(resolve_user(&req, &state).await?))
		})
	}
}
...
impl FromRequest for AdminUser {
	...
	fn from_request(req: &HttpRequest, _payload: &mut Payload) -> Self::Future {
		...
		Box::pin(async move {
			let state = app_state(&req);
			let user = resolve_user(&req, &state).await?;
...
async fn resolve_user(req: &HttpRequest, state: &AppState) -> std::result::Result<User, ApiError> {
	if let Some(user) = user_from_session(req, state).await? {
		return Ok(user);
	}
	if let Some(user) = user_from_api_key(req, state).await? {
		return Ok(user);
	}
...
async fn user_from_api_key(req: &HttpRequest, state: &AppState) -> Result<Option<User>> {
